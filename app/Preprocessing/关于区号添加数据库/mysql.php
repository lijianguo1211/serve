...
try{
    ...
    /*foreach ($option as $k => $v) {
        ...
        echo $v['en'];
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "INSERT INTO ui_area_code (city_name, area_code,city_name_en)VALUES (\"{$v['zh-cn']}\",\"{$k}\",\"{$v['en']}\")";
        $pdo->exec($sql);
        echo "新记录插入成功";
    }*/
    foreach ($options as $kk => $vv) {
        $sql = "select * from ui_area_code where area_code like \"{$vv}%\"";
        echo $sql;
        $stmt = $pdo->query($sql);
        ...
        foreach($stmt as $k=>$v) {
            $upsql = "UPDATE ui_area_code SET parent_city_name= \"{$kk}\" WHERE area_code=\"{$v['area_code']}\"";
            $pdo->exec($upsql);
            echo "success";
        }
    }
